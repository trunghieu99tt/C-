...
void trunghieu()
{
    ...
    for (auto &i : a)
        cin >> i;
    SegmentTreeMax stMax(a);
    SegmentTreeMin stMin(a);
    forn(i, n)
    {
        ...
        {
            int mid = (lo + hi) >> 1;
            int idMax = stMax.rmq(i, mid);
            int idMin = stMin.rmq(i, mid);
            int maxV = a[idMax];
            int minV = a[idMin];

            if (idMax < idMin && minV * 1.0 < maxV / 2.0)
            {
                hi = mid - 1;
            }
            else
        ...
        if (ans == n - 1)
        {
            ...
            {
                int mid = (lo + hi) >> 1;
                int idMax = stMax.rmq(mid, i);
                int idMin = stMin.rmq(mid, i);
                int maxV = a[idMax];
                int minV = a[idMin];
