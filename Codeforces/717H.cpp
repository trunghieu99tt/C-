...

{
...

    {
...

        {
            ...
            int len = wish_team[i].size();

            ans_team[i] = wish_team[i][rand() % len];
        }

        random_shuffle(begin(conferences) + 1, end(conferences));
...

        {
            ...
            if (conferences[ans_team[i[0]]] != conferences[ans_team[i[1]]])
