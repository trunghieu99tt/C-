...
void trunghieu()
{
    ...
    vll sum(n + 1, 0);
    for1(i, n) {
        sum[i] = sum[i - 1] + friends[i - 1].second;
    }
    ll ans = 0;
    for1(i, n) {
        int indexR = lower_bound(all(friends), make_pair(friends[i-1].first + d, numeric_limits<int>::min())) - begin(friends);
        if (indexR >= n || friends[indexR].first >= friends[i-1].first + d)
            indexR--;
        ans = max(ans, sum[indexR + 1] - sum[i-1]);
    }
    cout << ans;
}
