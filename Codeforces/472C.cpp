...

{
    ...
    string prev = name[p[0]].first;

    for (int i = 1; i < n; i++)

    {

        if (name[p[i]].first < prev && name[p[i]].second < prev)
        ...
        string sMax = max(name[p[i]].first, name[p[i]].second);

        string sMin = min(name[p[i]].first, name[p[i]].second);
