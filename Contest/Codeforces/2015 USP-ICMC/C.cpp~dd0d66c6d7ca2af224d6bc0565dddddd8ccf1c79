...
void trunghieu()
{
    ...
    x/=100;
    stringstream ss;
    ss<<setprecision(16)<<x;
    string s;
    ss>>s;
    int len = (s.find('.')!=string::npos)?s.size() - 1:s.size();
    ll n = ll(x);
    x-=n;
    const ll precision = 1e15;
    ll gcd = __gcd((ll)round(x * precision), precision);
    ll den = precision/ gcd;
    cout<<min(den,(ll)round(pow(10, len)));
}
